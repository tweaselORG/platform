...
export const POST: APIRoute = async ({ params, request, redirect }) => {
    ...
    try {
        ...
        if (question === 'askIsUserOfApp') {
            ...
            else set.complaintType = null; // This overwrites the old value, if there is one.
        ...
        const updatedProceeding = await e
            .select(updated, (proceeding) => ({
                complaintState: true,
            }))
            .run(client);
        ...
        return new Response('Unhandled internal error', { status: 500 });
    }
};
